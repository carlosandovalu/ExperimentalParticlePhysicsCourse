...
class ZAnalysis_8TeV(Analysis.Analysis):
  """Analysis searching for events where Z bosons decay to two leptons of same flavour and opposite charge.
  """
  def __init__(self, store):
    super(ZAnalysis_8TeV, self).__init__(store)
  
  def initialize(self):
      self.invMassZ              =  self.addStandardHistogram("invMassZ")
      ...
      self.hist_jetJVT          =  self.addStandardHistogram("jet_jvf")
      self.hist_jeteta          =  self.addStandardHistogram("jet_eta")      
      self.hist_jetmv2          =  self.addStandardHistogram("jet_MV1")
  ...
  def analyze(self):
      ...
      # Z boson Histograms
      self.invMassZ.Fill((leadLepton.tlv() + trailLepton.tlv()).M(), weight)
